...
<div class="row g-4">
    <div class="col-12 col-xl-8">
        <div class="admin-card rounded-4 bg-white p-4">
            ...
            <div class="text-secondary mb-3">{{ $item->slug }}</div>
            <p>{{ $item->short_desc }}</p>
            <div class="border-top pt-3">{!! nl2br(e($item->description)) !!}</div>
            <h3 class="h6 fw-bold mt-4">Detail</h3>
            <ul>@foreach ($item->details as $detail)<li>{{ $detail->detail }}</li>@endforeach</ul>
        </div>
    </div>
    <div class="col-12 col-xl-4">
        <div class="admin-card rounded-4 bg-white p-4">
            ...
            <div class="d-grid gap-2 mt-4">
                <a href="{{ route('admin.services.edit', $item) }}" class="btn btn-dark">Edit</a>
                <a href="{{ route('admin.services.index') }}" class="btn btn-light border">Kembali</a>
            </div>
        </div>
    </div>
</div>
